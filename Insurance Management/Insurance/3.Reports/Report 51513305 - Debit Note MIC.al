report 51513305 "Debit Note MIC"
{
    // version AES-INS 1.0

    DefaultLayout = RDLC;
    RDLCLayout = './Insurance/5.Layouts/Debit Note MIC.rdl';
    ...
    dataset
    {
        dataitem("Insure Debit Note"; "Insure Debit Note")
        {
            ...
            column(AppliedAmount_InsureDebitNote; "Insure Debit Note"."Applied Amount")
            {
            }
            ...
            column(EndorsementType_InsureDebitNote; "Insure Debit Note"."Endorsement Type")
            {
            }
            column(EndorsementPolicyNo_InsureDebitNote; "Insure Debit Note"."Endorsement Policy No.")
            {
            }
            ...
            begin
                ...
                IF "Insure Debit Note"."Applied Amount" <> 0 THEN BEGIN
                    Amount := "Insure Debit Note"."Total Net Premium" + "Insure Debit Note"."Applied Amount";
                    AppliedAmount := 'Credit Amount:' + '                   ' + FORMAT("Insure Debit Note"."Applied Amount");
                    AmountToPay := 'Amount To Pay:' + '                  ' + FORMAT(Amount);
                END;
                ...
                EmployeeFullName := '';
                EmpTitle := '';
                UserSetupDetails.SETRANGE(UserSetupDetails."User ID", "Insure Debit Note"."Created By");
                IF UserSetupDetails.FINDLAST THEN BEGIN
                    IF Emp.GET(UserSetupDetails.Employee) THEN BEGIN
                        EmpCopy.RESET;
                        EmpCopy.SETRANGE(EmpCopy.Title, 'General Manager');
                        IF EmpCopy.FINDFIRST THEN
                            GeneralManager := EmpCopy.FullName;
                        EmpCopy.RESET;
                        EmpCopy.SETRANGE(EmpCopy.Title, 'Senior Claims Manager');
                        IF EmpCopy.FINDFIRST THEN
                            SeniorClaimsManager := EmpCopy.FullName;
                ...
                CustLedger.RESET;
                CustLedger.SETRANGE(CustLedger."Document No.", "Insure Debit Note"."No.");
                /*
                IF CustLedger.FINDFIRST THEN
                  MESSAGE(FORMAT(CustLedger.Amount));
                */

                CustLedger.SETRANGE(CustLedger."Insurance Trans Type", CustLedger."Insurance Trans Type"::Commission);
                IF CustLedger.FINDLAST THEN BEGIN
                    CustLedger.CALCFIELDS(Amount);
                    Commission := CustLedger.Amount;
                END;

                CustLedger.RESET;
                CustLedger.SETRANGE(CustLedger."Document No.", "Insure Debit Note"."No.");
                CustLedger.SETRANGE(CustLedger."Insurance Trans Type", CustLedger."Insurance Trans Type"::Wht);
                IF CustLedger.FINDFIRST THEN BEGIN
                    CustLedger.CALCFIELDS(Amount);
                    Wht := CustLedger.Amount;
                END;
    ...
    begin
        ...
        AllFilters := "Insure Debit Note".GETFILTERS;
    end;
