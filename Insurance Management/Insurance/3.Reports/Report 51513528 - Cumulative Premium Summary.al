report 51513528 "Cumulative Premium Summary"
{
    // version AES-INS 1.0

    DefaultLayout = RDLC;
    RDLCLayout = './Insurance/5.Layouts/Cumulative Premium Summary.rdl';
    ...
    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            ...
            column(MonthlyGrossPremium; MonthlyGrossPremium)
            {
            }
            column(MonthlyStampDuty; MonthlyStampDuty)
            {
            }
            column(MonthlyCommissionAmount; MonthlyCommissionAmount)
            {
            }
            column(MonthlyWitholdingTax; MonthlyWitholdingTax)
            {
            }
            column(MontlhyTrainingLevy; MontlhyTrainingLevy)
            {
            }
            column(MonthlyPHCF; MonthlyPHCF)
            {
            }
            column(MonthlyNetPremium; MonthlyNetPremium)
            {
            }
            ...
            begin
                ...
                IF DimValue.GET(GeneralLedgSetup."Shortcut Dimension 3 Code", BusinessTypeCode) THEN BEGIN
                    DimValue.CALCFIELDS(DimValue."Gross Premium");
                    MonthlyGrossPremium := DimValue."Gross Premium";
                    DimValue.CALCFIELDS(DimValue."Stamp Duty");
                    MonthlyStampDuty := DimValue."Stamp Duty";
                    DimValue.CALCFIELDS(DimValue."Commission Amount");
                    MonthlyCommissionAmount := DimValue."Commission Amount";
                    DimValue.CALCFIELDS(DimValue."Withholding Tax");
                    // MonthlyWitholdingTax:=DimValue."Withholding Tax";
                    DimValue.CALCFIELDS(DimValue."Training Levy");
                    MontlhyTrainingLevy := DimValue."Training Levy";
                    DimValue.CALCFIELDS(DimValue.PCHF);
                    MonthlyPHCF := DimValue.PCHF;
                    DimValue.CALCFIELDS(DimValue."Net Premium");
                    MonthlyNetPremium := DimValue."Net Premium";
                END;
                ...
                IF PostedDebitNote.FIND('-') THEN BEGIN
                    ...
                    IF PolicyRec.GET(PostedDebitNote."Policy Type") THEN BEGIN
                        ClassOfInsurance := PolicyRec.Description;
                        ClassCode := PolicyRec.Code;
                    END;
                ...
                IF creditnote.FIND('-') THEN BEGIN
                    ...
                    IF PolicyRec.GET(creditnote."Policy Type") THEN BEGIN
                        ClassOfInsurance := PolicyRec.Description;
                        ClassCode := PolicyRec.Code;
                    END;
                ...
                "Cust. Ledger Entry".SETRANGE("Cust. Ledger Entry"."Insurance Trans Type", "Cust. Ledger Entry"."Insurance Trans Type"::Wht);
                IF "Cust. Ledger Entry".FINDFIRST THEN
                    MonthlyWitholdingTax := "Cust. Ledger Entry".Amount;
    ...
    var
        ...
        MonthlyGrossPremium: Decimal;
        MonthlyStampDuty: Decimal;
        MonthlyCommissionAmount: Decimal;
        MonthlyWitholdingTax: Decimal;
        MonthlyMiscDebs: Decimal;
        MontlhyTrainingLevy: Decimal;
        MonthlyPHCF: Decimal;
        MonthlyNetPremium: Decimal;
        ClassCode: Code[30];
