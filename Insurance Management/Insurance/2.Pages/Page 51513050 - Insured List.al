page 51513050 "Insured List"
{
    // version AES-INS 1.0

    CardPageID = "Insured Card";
    Editable = false;
    PageType = List;
    UsageCategory = Lists;
    SourceTable = Customer;
    SourceTableView = WHERE("Customer Type" = CONST(Insured), "Insured Type" = CONST(Individual));
        ...
        area(navigation)
        {
                ...
                action("DMS Link")
                {
                    ...
                    begin
                        IF InsSetup.GET THEN
                            //  HyperText:=InsSetup."DMS Link"+'?'+FORMAT("No.");
                            HyperText := InsSetup."Demand Letter Nos." + FORMAT("No.");
            ...
            group(Activity)
            {
                ...
                action("Reinstate Risk")
                {
                    ...
                    Promoted = true;
                    PromotedCategory = Process;
                    RunObject = Report ReinstateRisk;
                }
                action("Substitute Risk")
                {
                    ...
                    Promoted = true;
                    PromotedCategory = Process;
                    RunObject = Report SubstituteRisk;
                }
                action("Revise Risk")
                {
                    Caption = 'Revise Risk';
                ...
                action("Nil Endorsement Risk")
                {
                    ...
                    Promoted = true;
                    PromotedCategory = Process;
                    RunObject = Report NilEndorsementRisk;
                }
                action("Renew Risk")
                {
                    Caption = 'Renew Risk';
