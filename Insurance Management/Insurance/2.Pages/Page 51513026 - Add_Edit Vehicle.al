page 51513026 "Add_Edit Vehicle"
{
    // version AES-INS 1.0
    ...
    layout
    {
        area(content)
        {
            ...
            part("Additional Benefits"; "Additional Benefits")
            {
    ...
    begin
        ...
        IF InsureHeader.GET("Document Type", "Document No.") THEN BEGIN
            ...
            InsuranceClass.SETRANGE("Global Dimension No.", 3);
            InsuranceClass.SETRANGE(InsuranceClass.Code, InsureHeader."Policy Class");
            IF InsuranceClass.FindFirst then begin
                UnderWriterPolicyType.reset;
                UnderWriterPolicyType.setrange(UnderwriterPolicyType."Class of Insurance", InsuranceClass.Code);
                IF UnderWriterPolicyType.FindFirst() then begin
                    PolicyTypeTemplate.Reset();
                    PolicyTypeTemplate.setrange(PolicyTypeTemplate.Underwriter, UnderWriterPolicyType."Underwriter code");
                    PolicyTypeTemplate.setrange(PolicyTypeTemplate."Policy Type", UnderWriterPolicyType.Code);
                    if PolicyTypeTemplate.FindFirst() then
                       repeat
                      //Add visibility based on selected fields

                       until Policytypetemplate.next=0;
                end;
    ...
    var
        ...
        InsuranceClass: Record "Dimension Value";
        PolicyTypeTemplate: REcord "Policy Type Template";
}
