codeunit 51513000 "Insurance management"
{
    // version AES-INS 1.0
    ...
    //bkk mark
    procedure InsertTaxesReinsuranceB(var InsureHeader: Record "Insure Header");
    var
        InsureLoadingsDisc: Record "Insure Header Loading_Discount";
        insureLines: Record "Insure Lines";
        LastLineNo: Integer;
        ...
        AgentBrokerRec: Record Vendor;
        VATSetup: Record "VAT Posting Setup";
        Paymentschedule: Record "Instalment Payment Plan";
        CustomerTemplate: Record "Customer Template";
        CommissionDue: Decimal;
        MaturityInstalmentRec: Record "Maturity Instalment Ratio";
    ...
    begin
        ...
        InsureHeader.TESTFIELD(InsureHeader."Cover End Date");
        InsureHeader.TESTFIELD(InsureHeader."No. Of Instalments");
        InsureHeader.TESTFIELD(InsureHeader."Sales Agent");
        InsureHeader.TESTFIELD(InsureHeader.Underwriter);
        //InsureHeader.TESTFIELD(InsureHeader."Insurer Policy No");
        IF EndorsementType.GET(InsureHeader."Endorsement Type") THEN
            IF EndorsementType."Requires Cancellation Reason" THEN
                IF InsureHeader."Cancellation Reason" = '' THEN
                    ERROR('Please select a cancellation/Endorsment reason');
        ...
        //Insert Template
        //and message 
        //MESSAGE('before =%1',Insureheader."Insure Template Code");
        if Insureheader."Insure Template Code" = '' THEN begin
            CustomerTemplate.RESET;
            IF CustomerTemplate.FINDFIRST THEN BEGIN
                InsureHeader."Insure Template Code" := CustomerTemplate.Code;
                InsureHeader.MODIFY;
                //message('Inserted Template %1',InsureHeader."Insure Template Code" );
            END;
        end;
        ...
        IF insureLines.FINDFIRST THEN
            REPEAT
                ...
                //IntegrationMapp.RESET;
                //IntegrationMapp.SETRANGE(IntegrationMapp."Class Code", insureLines."Policy Type");
                IF InsureHeader.UnderWriter <> '' THEN BEGIN
                    insureLines."Account Type" := insureLines."Account Type"::Vendor;
                    insureLines."Account No." := InsureHeader.Underwriter;
                END;
                insureLines.VALIDATE(insureLines."Account No.");
                //MESSAGE('Account type %1 and Account no=%2',insureLines."Account Type",IntegrationMapp."Gross Premium Account");
                IF insureLines."No. Of Instalments" <> 0 THEN BEGIN
                ...
                if InsureHeader."Commission Due" = 0 then
                    InsureHeader."Commission Due" := PolicyTypeRec."Commision % age(SIIBL)";
                //Apply Instalment ratio if it exists on the setup for the policy in question
                ...
                insureLines."Insured No." := InsureHeader."Insured No.";
                insureLines."Insured Name" := InsureHeader."Insured Name";
                If insurelines.GET(Insurelines."Document Type", InsureLines."Document No.", Insurelines."Line No.", Insurelines."Risk Id") then
                    insureLines.MODIFY();
            UNTIL insureLines.NEXT = 0;
        ...
        IF PolicyTypeRec.GET(InsureHeader."Policy Type", InsureHeader.Underwriter) THEN
        begin
            IF PolicyTypeRec.Comprehensive THEN
                DrawPaymentScheduleComp(InsureHeader)
            ELSE
        ...
        //for Policies with Maturities generate a Maturity Payment schedule
        IF PolicyTypeRec."Partial Maturity?" then 
        Begin
            MaturityInstalmentRec.reset;
            MaturityInstalmentRec.setrange("Underwriter Code",PolicyTypeRec."Underwriter Code");
            MaturityInstalmentRec.setrange("Policy Type",PolicyTypeRec.Code);
            If MaturityInstalmentRec.FindLast then
            DrawMaturityScheduleDefined(InsureHeader)
            else
            DrawMaturityPaymentSchedule(InsureHeader);
        ...
        InsuranceSetup.GET;
        IF InsureHeader."Sales Agent" <> '' THEN BEGIN
            //InsureHeader.VALIDATE(InsureHeader."Sales Agent");
            //InsureHeader.MODIFY;
            //MESSAGE('Test =%1 %2',InsureHeader."Sales Agent",InsureHeader."Commission Payable 1" );

            CommissionAmt := ROUND(InsureHeader."Total Premium Amount" * (InsureHeader."Commission Payable 1" / 100), 1);
            IF InsureHeader."Total Premium Amount" <> 0 THEN BEGIN
                IF CommissionAmt = 0 THEN
                    ERROR('Please Check on Commission Percentage definition');
            END;
            ...
            CommissionDue := ROUND(InsureHeader."Total Premium Amount" * (InsureHeader."Commission Due" / 100), 1);
            ...
            PartnerLines."No." := InsureHeader."No.";
            PartnerLines."Transaction Type" := PartnerLines."Transaction Type"::"Broker ";
            PartnerLines."Account Type" := PartnerLines."Account Type"::"Vendor";
            PartnerLines."Partner No." := InsureHeader."Sales Agent";
            //PartnerLines."Account No.":=InsuranceSetup."Withholding Tax Account";
            //MESSAGE('%1',InsureHeader."Agent/Broker");
            PartnerLines.VALIDATE(PartnerLines."Partner No.");
            PartnerLines."Broker Commission" := CommissionAmt;
            PartnerLines."WHT Amount" := WitholdingTax;
            PartnerLines."Commission Type" := Partnerlines."Commission Type"::Outwards;
            //MESSAGE('Inserting Commission');
            PartnerLines.INSERT;
            //
            ...
            PartnerLines."Transaction Type" := PartnerLines."Transaction Type"::"Broker ";
            PartnerLines."Account Type" := PartnerLines."Account Type"::"Vendor";
            PartnerLines."Partner No." := InsureHeader.Underwriter;
            //PartnerLines."Account No.":=InsuranceSetup."Withholding Tax Account";
            //MESSAGE('%1',InsureHeader."Agent/Broker");
            PartnerLines.VALIDATE(PartnerLines."Partner No.");
            PartnerLines."Broker Commission" := CommissionDue;
            PartnerLines."WHT Amount" := 0;
            PartnerLines."Commission Type" := PartnerLines."Commission Type"::Inwards;
            //MESSAGE('Inserting Commission due %1 %2',InsureHeader."Commission Due",CommissionDue);
            PartnerLines.INSERT;
            ...
            PartnerLines."Document Type" := InsureHeader."Document Type";
            PartnerLines."No." := InsureHeader."No.";
            PartnerLines."Transaction Type" := PartnerLines."Transaction Type"::"Co-insurance";
            PartnerLines."Account Type" := PartnerLines."Account Type"::"Vendor";
            PartnerLines."Partner No." := InsureHeader."Underwriter";
            //PartnerLines."Account No.":=InsuranceSetup."Withholding Tax Account";
            //MESSAGE('%1',InsureHeader."Agent/Broker");
            InsureHeader.CALCFIELDS(InsureHeader."Total Premium Amount", "Total Net Premium");
            PartnerLines.VALIDATE(PartnerLines."Partner No.");
            PartnerLines.Premium := InsureHeader."Total Net Premium";
            PartnerLines."Coinsurance Percentage" := 100;
            //MESSAGE('Inserting Underwriter Premium');
            PartnerLines.INSERT;
    ...
    //bkk -add taxes brokerage
    procedure InsertTaxationB(var SalesHeadr: Record "Insure Header");
    var
    ...
    begin
        ...
        IF PolicyType.GET(InsureHeader."Policy Type", InsureHeader.Underwriter) THEN BEGIN
    ...
    begin
        ...
        IF PolicyHeader."No." <> '' THEN BEGIN
            IF PaymentTerms.GET(PolicyHeader."No. Of Instalments") THEN BEGIN
                IF PaymentTerms."No. Of Instalments" <> 0 THEN BEGIN

                    StartDate := PolicyHeader."From Date";
                    IF PolicyHeader."From Date" = 0D THEN
                        ERROR('Please key in the policy start date');
                    IF TermRec.GET(PolicyHeader.Term) THEN
                        No_Of_Instalments := PaymentTerms."No. Of Instalments" * TermRec."No. Of Periods";

                    //MESSAGE('No of Instalments=%1', No_Of_Instalments);

                    FOR i := 1 TO No_Of_Instalments DO BEGIN
                        ...
                        PolicyHeader.CALCFIELDS(PolicyHeader."Total Premium Amount", PolicyHeader."Total Net Premium");
                        PaymentSchedule."Amount Due" := PolicyHeader."Total Net Premium";
                        IF i = 1 THEN BEGIN
    ...
    procedure DrawMaturityPaymentSchedule(var PolicyHeader: Record "Insure Header");
    var
        PaymentSchedule: Record "Payment Instalment Plan";
        TotalPremium: Decimal;
        PolicyLines: Record "Insure Lines";
        StartDate: Date;
        i: Integer;
        ...
        TermRec: Record Term;
        No_Of_Instalments: Integer;
        MaturityInstalRatio: Record "Maturity Instalment Ratio";
    begin
        //MESSAGE('Running Schedule');

        MaturityInstalRatio.reset;
        MaturityInstalRatio.setrange(MaturityInstalRatio."Underwriter Code",PolicyHeader.Underwriter);
        MaturityInstalRatio.setrange(MaturityInstalRatio."Policy Type",PolicyHeader."Policy Type");
        If Not MaturityInstalRatio.Findlast then
        Begin
        ...
        IF PolicyHeader."No." <> '' THEN BEGIN
            IF PaymentTerms.GET(PolicyHeader."Pay-out Frequency") THEN BEGIN
                IF PaymentTerms."No. Of Instalments" <> 0 THEN BEGIN

                    StartDate := PolicyHeader."Pay-out Commencement Date";
                    IF PolicyHeader."Pay-out Commencement Date" = 0D THEN
                        ERROR('Please key in the Pay-out Commencement date');
                    IF TermRec.GET(PolicyHeader.Term) THEN
                        No_Of_Instalments := PaymentTerms."No. Of Instalments" * TermRec."No. Of Periods";
                   ...
                   // FOR i := 1 TO No_Of_Instalments DO BEGIN
                       //bkk use while..
                       While StartDate<=PolicyHeader."To Date" DO BEGIN
                        PaymentSchedule.INIT;
                        PaymentSchedule."Document Type" := PolicyHeader."Document Type";
                        PaymentSchedule."Document No." := PolicyHeader."No.";
                        PaymentSchedule."Payment No" := i;
                        ...
                        // PaymentSchedule."Amount Due":=TotalPremium/PaymentTerms."No. of Payments";
                       // PolicyHeader.CALCFIELDS(PolicyHeader."Total Premium Amount", PolicyHeader."Total Net Premium");
                        PaymentSchedule."Amount Due" := PolicyHeader.Annuity;
                        IF i = 1 THEN BEGIN
                            PaymentSchedule."Due Date" := StartDate;
                            PaymentSchedule."Cover Start Date" := StartDate;
                            PaymentSchedule."Cover End Date" := CALCDATE(PaymentTerms."Period Length", PaymentSchedule."Cover Start Date") - 1
    ...
    //bkk 17.03.21
    procedure ConvertQuote2DebitNoteB(var InsureHeader: Record "Insure Header");
    var
        Reinslines: Record "Coinsurance Reinsurance Lines";
        InsureLines: Record "Insure Lines";
        AdditionalBenefits: Record "Additional Benefits";
    ...
    begin
        ...
        ELSE
         ...
         InsureHeaderCopy."Policy No" := InsureHeader."No."; */ //BKK 18-03-21
        InsureHeaderCopy.VALIDATE("Insured No.");
        InsureHeaderCopy.INSERT(TRUE);
        ...
        IF InstalmentLines.FINDFIRST THEN
            ...
            UNTIL InstalmentLines.NEXT = 0;
        //Post Debit Note
        PostInsureHeaderB(InsureHeaderCopy);
    end;
    ...
    begin
        ...
        InsureLines.SETRANGE(InsureLines."Document Type", InsureHeader."Document Type");
        InsureLines.SETRANGE(InsureLines."Document No.", InsureHeader."No.");
        InsureLines.SETFILTER(InsureLines."Line No.", '<>%1', 0);
        IF InsureLines.FINDFIRST THEN
            REPEAT
                InsureLinesCopy.INIT;
    ...
    //Bkk
    procedure ConvertQuote2PolicyB(var InsureHeader: Record "Insure Header");
    var
        Reinslines: Record "Coinsurance Reinsurance Lines";
        InsureLines: Record "Insure Lines";
        AdditionalBenefits: Record "Additional Benefits";
    ...
    begin
        ...
        IF InsureHeader."Insured No." = '' THEN BEGIN
            IF Contact.GET(InsureHeader."Contact No.") THEN BEGIN
                ContBusRelation.reset;
                ContBusRelation.Setrange("Contact No.", InsureHeader."Contact No.");
                ContBusRelation.setrange(ContBusRelation."Link to table", 1);
                IF ContBusRelation.FINDLAST THEN begin
                    InsureHeader."Insured No." := ContBusRelation."No.";
                    InsureHeader.VALIDATE("Insured No.");
                    // MESSAGE('Start Contact relation customer=%1', ContBusRelation."No.");
                    InsureHeader.MODIFY;
                end
                else BEGIN
                    IF InsureHeader."Insure Template Code" = '' THEN begin
                        ...
                        //MESSAGE('Template %1',InsureHeader."Insure Template Code");

                    end;
                    //message('creating x customer');
                    If InsureHeader."Insured No." = '' then
                        Contact.CreateInsured(InsureHeader."Insure Template Code");
                    //MESSAGE('Insured=%1 and no.=%2',Insureheader."Insured No.",InsureHeader."No.");
                    InsureHeader.VALIDATE("Insured No.");
                    InsureHeader.MODIFY;
        ...
        InsureHeaderCopy.COPY(InsureHeader);
        IF InsureHeaderCopy.Status <> InsureHeaderCopy.Status::Released THEN
            ERROR('%1 %2 m ust be fully approved before being converted to a policy', InsureHeaderCopy."Document Type", InsureHeaderCopy."No.");

        InsureHeaderCopy."Document Type" := InsureHeaderCopy."Document Type"::Policy;
        InsureHeaderCopy."No." := GeneratePolicyNos(InsureHeaderCopy);
        //MESSAGE('Policy No=%1',GeneratePolicyNos(InsureHeaderCopy));

        IF InsureHeaderCopy."Insured No." = '' THEN BEGIN
            IF Contact.GET(InsureHeaderCopy."Contact No.") THEN BEGIN

                /*Cust.RESET;
                Cust.SETRANGE(Cust."ID/Passport No.", Contact."ID/Passport No.");
                ...
                END;*/
                ContBusRelation.reset;
                ContBusRelation.Setrange("Contact No.", InsureHeaderCopy."Contact No.");
                ContBusRelation.setrange(ContBusRelation."Link to table", 1);
                IF ContBusRelation.FINDLAST THEN begin
                    InsureHeaderCopy."Insured No." := ContBusRelation."No.";
                    InsureHeaderCopy.VALIDATE("Insured No.");
                    //MESSAGE('Contact relation customer=%1', ContBusRelation."No.");
                    //InsureHeaderCopy.MODIFY;
                end
                else BEGIN
                    IF InsureHeaderCopy."Insure Template Code" = '' THEN begin
                        CustomerTemplate.RESET;
                        IF CustomerTemplate.FINDFIRST THEN
                            InsureHeaderCopy."Insure Template Code" := CustomerTemplate.Code;
                        //  InsureHeaderCopy.MODIFY;
                    ...
                    If InsureHeaderCopy."Insured No." = '' then
                        Contact.CreateInsured(InsureHeaderCopy."Insure Template Code");
                    //MESSAGE('Insured=%1',InsureheaderCopy."Insured No.");
                    //InsureHeaderCopy.MODIFY;
    ...
    begin
        ...
        IF InsureHeader."Policy No" = '' THEN
            InsureHeader."Policy No" := InsureHeader."Quotation No.";
    ...
    begin
        ...
        IF InsureHeader."Document Type" = InsureHeader."Document Type"::"Debit Note" THEN
            IF InsureHeader.CheckCustomerCreated(TRUE) THEN
                InsureHeader.GET(InsureHeader."Document Type"::"Debit Note", InsureHeader."No.")
            ELSE
                EXIT;

        InsureHeader.TESTFIELD(InsureHeader."Document Type", InsureHeader."Document Type"::"Debit note");
        ...
        */
        IF InsureHeader."Insured No." = '' THEN BEGIN
            IF Contact.GET(InsureHeader."Contact No.") THEN BEGIN
                ...
                Cust.SETRANGE(Cust."ID/Passport No.", Contact."ID/Passport No.");
                IF Cust.FINDLAST THEN BEGIN
                    //InsureHeader."Insured No." := Cust."No.";
                    // InsureHeader.VALIDATE(InsureHeader."Insured No.");*/
                    MEssage('found');
                ...
                IF ContBusRelation.FINDLAST THEN begin
                    ...
                    //message('creating customer');
                    Contact.CreateInsured(InsureHeaderCopy."Insure Template Code");
                    //MESSAGE('Insured=%1',Insureheader."Insured No.");
                end;
        ...
        //
        // MESSAGE('Insured before journal=%1',InsureHeader."Insured No.");
        ...
        IF PolicyType.GET(InsureHeader."Policy Type", InsureHeader.Underwriter) THEN
            IF PolicyType."Premium Payment options" = PolicyType."Premium Payment options"::"Pay Broker" then begin
                TotalInvoice := 0;
                Isetup.GET;
                Isetup.TESTFIELD(Isetup."Insurance Template");
                GenBatch.INIT;
                ...
                GenJnlLine.VALIDATE(GenJnlLine.Amount);
                */
                TotalInvoice := TotalInvoice + InsureHeader."Total Premium Amount";
                /*
                ...
                IF InsureLines.FINDFIRST THEN
                    REPEAT
                        ...
                        GenJnlLine."Insured ID" := InsureHeader."Insured No.";
                        */
                        TotalInvoice := TotalInvoice + InsureLines.Amount;
                    /*GenJnlLine."Shortcut Dimension 1 Code":=InsureHeader."Shortcut Dimension 1 Code";
                    GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 1 Code");
                ...
                IF InsureHeader."Premium Finance %" = 0 THEN BEGIN
                    ...
                    ELSE
                        GenJnlLine."Account No." := InsureHeader."Insured No.";
                    IF InsureHeader."Insured No." = '' THEN
                        ERROR('Insured must be created');
                    // MESSAGE('Insured on Journal line=%1',InsureHeader."Insured No." );
                    //GenJnlLine."Document Type":=GenJnlLine."Document Type"::Invoice;
                    GenJnlLine.VALIDATE(GenJnlLine."Account No.");
                    GenJnlLine.Description := COPYSTR(STRSUBSTNO('%1 POLICY No. %2', InsureHeader."Insured Name", InsureHeader."Insurer Policy No"), 1, 50);
                    ...
                    GenJnlLine."Insurance Trans Type" := GenJnlLine."Insurance Trans Type"::"Net Premium";
                    GenJnlLine."Policy No" := InsureHeader."Insurer Policy No";
                    IF InsureHeader."Endorsement Policy No." = '' THEN
                        GenJnlLine."Endorsement No." := InsureHeader."Insurer Policy No"
                    ELSE
                        GenJnlLine."Endorsement No." := InsureHeader."Endorsement Policy No.";
                    GenJnlLine."Policy Type" := InsureHeader."Policy Type";
                    GenJnlLine."Shortcut Dimension 3 Code" := InsureHeader."Shortcut Dimension 3 Code";
                    ...
                    IF GenJnlLine.Amount <> 0 THEN
                        GenJnlLine.INSERT;
                    //Message('inserting customer leg');
                END;
                ;
                /*    ELSE BEGIN
                        LoadingDisc.RESET;
                ...
                IF CoInsureReinsureBrokerLines.FINDFIRST THEN
                    REPEAT
                        ...
                        IF CoInsureReinsureBrokerLines."Transaction Type" = CoInsureReinsureBrokerLines."Transaction Type"::"Broker " THEN BEGIN
                            IF PolicyType."Commission Calculation Basis" = PolicyType."Commission Calculation Basis"::"Booked Business" THEN BEGIN
                        ...
                        IF CoInsureReinsureBrokerLines."Transaction Type" = CoInsureReinsureBrokerLines."Transaction Type"::"Co-insurance" THEN BEGIN
                            ...
                            GenJnlLine."Posting Date" := InsureHeader."Posting Date";
                            GenJnlLine."Document No." := InsureHeader."Posting No.";
                            GenJnlLine."Account Type" := CoinsureReinsureBrokerLines."Account Type";
                            GenJnlLine."Account No." := CoInsureReinsureBrokerLines."Partner No.";
                            CoInsureReinsureBrokerLines.TESTFIELD(CoInsureReinsureBrokerLines."Partner No.");
                            GenJnlLine.VALIDATE(GenJnlLine."Account No.");
                            //GenJnlLine."Document Type":=GenJnlLine."Document Type"::Invoice;
                            GenJnlLine.Description := COPYSTR(STRSUBSTNO('%1 POLICY No. %2', InsureHeader."Insured Name", InsureHeader."Insurer Policy No"), 1, 50);
                            IF InsureHeader."Document Type" = InsureHeader."Document Type"::"Debit Note" THEN
                                GenJnlLine.Amount := -(TotalInvoice * (CoInsureReinsureBrokerLines."Coinsurance Percentage" / 100));
                            IF InsureHeader."Document Type" = InsureHeader."Document Type"::"Credit Note" THEN
                                GenJnlLine.Amount := TotalInvoice * (CoInsureReinsureBrokerLines."Coinsurance Percentage" / 100);
                            GenJnlLine.VALIDATE(GenJnlLine.Amount);
                            GenJnlLine."Shortcut Dimension 1 Code" := InsureHeader."Shortcut Dimension 1 Code";
                            ...
                            IF GenJnlLine.Amount <> 0 THEN
                                GenJnlLine.INSERT;
                            //MESSAGE('Inserting Coinsurance partner part');
        ...
        BEGIN
            ...
            IF InsureLines.FINDFIRST THEN
                REPEAT
                    IF InsureHeader."Document Type" = InsureHeader."Document Type"::"Debit Note" THEN BEGIN
                        ...
                        CertificateforPrint.Amount := PostedDrLines.Amount;
                        CertificateforPrint."Policy Type" := PostedDrLines."Policy Type";
                        CertificateforPrint."Policy No" := InsureHeader."Insurer Policy No";
                        CertificateforPrint."Start Date" := InsureHeader."Cover Start Date";
                        CertificateforPrint."End Date" := InsureHeader."Cover End Date";
                        CertificateforPrint."No. Of Instalments" := PostedDrLines."No. Of Instalments";
                        CertificateforPrint."Chassis No." := PostedDrLines."Chassis No.";
            ...
            BEGIN
                ...
                InsureLines.DELETEALL;*/
                PostedDrHeader.SETRECFILTER;
                //Commented by Bkk check report selection Activate later
                //PostedDrHeader.PrintRecords(FALSE);
    ...
    procedure CreateMaturityInstalmentRatio(var PolicyINstal: Record "Maturity Instalments Setup");
    var
        InstalmentRatio: Record "Maturity Instalment Ratio";
        i: Integer;
    begin
        InstalmentRatio.RESET;
        InstalmentRatio.SETRANGE(InstalmentRatio."Policy Type", PolicyINstal."Policy Type");
    ...
    procedure DrawMaturityScheduleDefined(var PolicyHeader: Record "Insure Header");
    var
        PaymentSchedule: Record "Payment Instalment Plan";
        TotalPremium: Decimal;
        PolicyLines: Record "Insure Lines";
        ...
        i: Integer;
        PaymentTerms: Record "No. of Instalments";
        InstalmentRatioRec: Record "Maturity Instalment Ratio";
    begin
        //MESSAGE('Running comprehensive');
        WITH PolicyHeader DO BEGIN
            CALCFIELDS("Total Comprehensive Premium", "Total TPO Premium", "Total PLL Premium", "Total Extra Premium", "Total Tax",
            ...
            IF "Pay-out Commencement Date" <> 0D THEN BEGIN  //Read from Instalment Percentage setups
                PaymentSchedule.RESET;
                PaymentSchedule.SETRANGE(PaymentSchedule."Document Type", "Document Type");
                PaymentSchedule.SETRANGE(PaymentSchedule."Document No.", "No.");
                PaymentSchedule.DELETEALL;
                ...
                InstalmentRatioRec.SETRANGE(InstalmentRatioRec."No. Of Instalments", "No. Of Instalments");
                IF InstalmentRatioRec.FINDFIRST THEN BEGIN
                    StartDate := "Pay-out Commencement Date";
                    REPEAT
                        PaymentSchedule.INIT;
                        PaymentSchedule."Document Type" := "Document Type";
                        PaymentSchedule."Document No." := "No.";
                        ...
                        StartDate := CALCDATE(InstalmentRatioRec."Period Length", StartDate);
                        PaymentSchedule."Instalment Percentage" := InstalmentRatioRec.Percentage;
                        PaymentSchedule."Amount Due" := (PaymentSchedule."Instalment Percentage" / 100) * ("Total Sum Insured");
                        PaymentSchedule.INSERT;
                    UNTIL InstalmentRatioRec.NEXT = 0;
                END;
            END;
    ...
    /*procedure DrawPostedDatedChequesPV(var PV: Record Payments);
    var
        PaymentSchedule: Record "Instalment Payment Plan";
        TotalPremium: Decimal;
        PolicyLines: Record "Insure Lines";
    ...
    procedure DrawPostedDatedChequesRec(var ReceiptHeader: Record "Receipts HeaderX");
    var
        PaymentSchedule: Record "Instalment Payment Plan";
        TotalPremium: Decimal;
        PolicyLines: Record "Insure Lines";
    ...
    var
        DayMonth: Code[10];
        //MessageTypeRec: Record "Notification Template"; SNN
    ...
    begin
        ...
        END
        ELSE
            ERROR('Notification message for Application confirmation..Not set');*/ //SNN
    end;
    ...
    procedure AgeCalculator(var BirthDate: Date; var ReferenceDate: Date) ageatRetire: Integer;
    var
        memberec: Record Vendor;
        NextYr: Date;
    begin
        ...
        BEGIN
            IF BirthDate <> 0D THEN BEGIN
                ...
                WHILE NextYr <= ReferenceDate DO BEGIN
                    NextYr := CALCDATE('1Y', NextYr);
                    ageatRetire := ageatRetire + 1;
                END;
                ageatRetire := ageatRetire - 1;
                //UNTIL NextYr>=WORKDATE;
            END;
        END;
    end;

    procedure CalcPremiumLife(var InsureHeader: Record "Insure Header");
    var
        InsureLoadingsDisc: Record "Insure Header Loading_Discount";
        insureLines: Record "Insure Lines";
        LastLineNo: Integer;
        ...
        VATSetup: Record "VAT Posting Setup";
        Paymentschedule: Record "Instalment Payment Plan";
        ProductBenRiders: Record 51513150;
        InsureCoverSelection: Record 51513161;
        AdditionalBenefitPremium: Record "Additional Benefits";
    begin
        CheckValidity(InsureHeader);
        InsureHeader.TESTFIELD(InsureHeader."Policy Type");
        InsureHeader.TESTFIELD(InsureHeader."From Date");
        ...
        AdditionalBenefitPremium.RESET;
        AdditionalBenefitPremium.SETRANGE(AdditionalBenefitPremium."Document Type", InsureHeader."Document Type");
        AdditionalBenefitPremium.SETRANGE(AdditionalBenefitPremium."Document No.", InsureHeader."No.");
        AdditionalBenefitPremium.DELETEALL;
        //Move here
        insureLines.SETRANGE(insureLines."Document Type", InsureHeader."Document Type");
        insureLines.SETRANGE(insureLines."Document No.", InsureHeader."No.");
        insureLines.SETRANGE(insureLines."Description Type", insureLines."Description Type"::"Schedule of Insured");
        ...
        IF insureLines.FINDFIRST THEN
            REPEAT
                ...
                //MESSAGE('Account type %1 and Account no=%2',insureLines."Account Type",IntegrationMapp."Gross Premium Account");
                IF insureLines."No. Of Instalments" <> 0 THEN BEGIN
                    insureLines.VALIDATE(insureLines.Age);
                    insureLines.MODIFY;
                END;

                //BKK-Introduce Premium for Riders
                InsureCoverSelection.RESET;
                InsureCoverSelection.SETRANGE(InsureCoverSelection."Document Type", insureLines."Document Type");
                InsureCoverSelection.SETRANGE(InsureCoverSelection."No.", insureLines."Document No.");
                InsureCoverSelection.SETRANGE(InsureCoverSelection.Selected, TRUE);
                IF InsureCoverSelection.FINDFIRST THEN BEGIN

                    REPEAT
                        AdditionalBenefitPremium.INIT;
                        AdditionalBenefitPremium."Document Type" := insureLines."Document Type";
                        AdditionalBenefitPremium."Document No." := insureLines."Document No.";
                        AdditionalBenefitPremium."Risk ID" := insureLines."Line No.";
                        AdditionalBenefitPremium."Option ID" := InsureCoverSelection.Code;
                        AdditionalBenefitPremium.Description := InsureCoverSelection.Description;
                        AdditionalBenefitPremium."Sum Insured" := insureLines."Sum Insured";
                        AdditionalBenefitPremium."Loading Amount" := InsureCoverSelection."Loading Amount";
                        AdditionalBenefitPremium.Premium := InsureCoverSelection."Loading Amount";
                        AdditionalBenefitPremium.INSERT;
                    UNTIL InsureCoverSelection.NEXT = 0;
                END;

                //Bkk

                InsureHeader."Premium Calculation Basis" := InsureHeader."Premium Calculation Basis"::"Full Premium";
                ...
                /*InstalmentRatioRec.RESET;
                            ...
                            InstalmentRatioRec.SETRANGE(InstalmentRatioRec."Instalment No", InsureHeader."Instalment No.");
                            InstalmentRatioRec.SETRANGE(InstalmentRatioRec."No. Of Instalments", InsureHeader."No. Of Instalments");
                            IF InstalmentRatioRec.FINDLAST THEN BEGIN
                                //MESSAGE('Instalment Ratio set');
                                IF InstalmentRatioRec.Percentage = 0 THEN
                                    ERROR('Please specify the percentage ratio for %1 instalment plan', insureLines."Policy Type");
                                insureLines.Amount := (insureLines."Gross Premium" + insureLines."Extra Premium" + insureLines."TPO Premium" + insureLines.PLL) *
