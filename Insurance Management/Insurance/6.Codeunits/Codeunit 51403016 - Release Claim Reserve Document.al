codeunit 51403016 "Release Claim Reserve Document"
{
    // version AES-INS 1.0
    ...
    procedure Reopen(var ClaimReserve: Record "Claim Reservation Header");
    begin
        OnBeforeReopenClaimDoc(ClaimReserve);

        WITH ClaimReserve DO BEGIN
        ...
        OnAfterReopenClaimDoc(ClaimReserve);

    end;

    procedure PerformManualRelease(var ClaimReserve: Record "Claim Reservation Header");
    var
        PrepaymentMgt: Codeunit "Prepayment Mgt.";
    ...
    begin
        ...
        //IF ApprovalsMgmt.IsClaimReserveApprovalsWorkflowEnabled(ClaimReserve) AND (ClaimReserve.Status = ClaimReserve.Status::Open) THEN
        ERROR(Text002);

        CODEUNIT.RUN(CODEUNIT::"Release Claim Reserve Document", ClaimReserve);

    end;

    procedure PerformManualReopen(var ClaimReserve: Record "Claim Reservation Header");
    begin
        IF ClaimReserve.Status = ClaimReserve.Status::"Pending Approval" THEN
            ERROR(Text003);
    ...
    local procedure OnBeforeReleaseClaimDoc(var ClaimReserve: Record "Claim Reservation Header");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterReleaseClaimDoc(var ClaimReserve: Record "Claim Reservation Header");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeReopenClaimDoc(var ClaimReserve: Record "Claim Reservation Header");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterReopenClaimDoc(var ClaimReserve: Record "Claim Reservation Header");
    begin
    end;
}
